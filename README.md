...
Each entry in the chart specifies the task name, the assigned team members, and the specific dependencies required for task initiation. While this figure serves as a summary of our workflow, a high-resolution, full-scale version of the Gantt chart is provided in [*Figure 16 - Gantt chart timeline for the DPM project*](#figure-16). The appendix version offers a comprehensive view of the entire project duration and more granular detail for each scheduled phase.
...
| :---- | ----- |
| **People present/Testers**: | J. SauvéA. Savard |
| **Author**: | S. Gelfand |
...
| **Procedure** (numbered steps): | Connect motor with wheel to the BrickPi Mark a 'zero' reference point on the wheel Run rotation for the specified target distance Record actual distance covered by rotation using the reference point Compare with encoded digital distance Calculate and record error Repeat with varying distances (3 total: 5cm, 10cm, 20cm) Repeat with different motors (EV3 Large Motor 1, EV3 Large Motor 2, EV3 Large Motor 3, EV3 Medium Servo Motor) *Note: predicted distance was calculated by multiplying distance per encoder count (wheel circumference ÷ 720 ticks/revolution) by the number of recorded encoder iterations.* |
...
| **Report/Outcomes**: |  Motor Trial 1: 5cm  Trial 2: 10cm  Trial 3: 20cm  Avg. Error  Digital Distance Error Digital Distance Error Digital Distance Error EV3 Large Motor 1 (95658) 4.9cm 0.1cm 10.2cm 0.2cm 21.1cm 1.1cm 0.47cm EV3 Large Motor 2 (95658) 5.0cm 0.0cm 9.9cm 0.1cm 21.5cm 1.5cm 0.53cm EV3 Large Motor 3 (95658) 4.8cm 0.2cm 11.1cm 1.1cm 21.7cm 1.7cm 1.00cm EV3 Medium Servo Motor (45503) 5.1cm 0.1cm 11.2cm 1.2cm 22.2cm 2.2cm 1.17cm ⇒ Overall: all four motors recorded average errors **within the 2cm threshold** ⇒ test **passed**. |
...
| **Who is this for** (which leads/managers): | Project ManagerDrive Subteam Lead |
...
| **Procedure** (numbered steps): | Sensor placed at a fixed starting position Sensor physically rotated to a specified target angle Trial performed clockwise Trial repeated counter-clockwise Digital output values and recorded angular distance evaluated post-rotation Above steps repeated for three different rotation magnitudes Average error calculated across all trials and directions Error assessed against two-degree buffer threshold *Note: no complex calculations were required for this test; digital values are processed in degrees relative to the sensor's boot-up position.* |
...
| **Who is this for** (which leads/managers): | Project ManagerDrive Subteam LeadVision Subteam Lead |
...
| **Procedure** (numbered steps): | **1 Touch Sensor Click Test** Securely connect one of the two touch sensors to port S1 of the BrickPi using an RJ45 cable Power the BrickPi with a fully charged 12V lithium-ion battery for the duration of the procedure Run *touch_and_speaker_test.py* and select reliability testing mode ("r") filepath: source/project/touch_and_speaker_test.py Actuate the sensor manually with normal pressure and observe any visible latency; verify that the number of inputs matches the number of clicks registered in the program output Repeat steps a) to d) for the second touch sensor Compare results  **2 Touch Sensor Speaker Test** Repeat steps a) and b) above Connect the speaker to the BrickPi's audio jack output Run *touch_and_speaker_test.py* and select speaker sound testing mode ("s") One team member actuates the touch sensor while another simultaneously starts a stopwatch; stop the stopwatch when the output sound is emitted and tabulate the latency Repeat steps a) to d) for the second touch sensor Compare results |
...
| :---- | :---- |
| **People present/Testers**: | J. SauvéZ. ChuangI. Gaspart |
| **Author**: | I. Gaspart |
...
| **Procedure** (numbered steps): | Sensor placed over a target colour X Colour detecting code placed in debugging mode (i.e. R, G, B channel values and detected colour printed to terminal) If no colour is detected or detection oscillates, adjust the RGB interval boundaries for colour X accordingly If the colour detected is incorrect, refine the RGB thresholds until correct classification is achieved Repeat step 2 three times until colour X is correctly detected approximately 100% of the time Repeat for all five colours (red, orange, blue, yellow, green) Place sensor over a white surface and observe output; repeat for black surface |
...
| :---- | :---- |
| **People present/Testers**: | J. SauvéI. GaspartZ. Chuang |
| **Author**: | I. Gaspart |
...
| **Test Objectives** [if applicable - specific, itemized & measurable]: | This test is for accuracy and reliability of the colour subsystem composed of the colour detecting code and the colour sensor. Accuracy must be close to perfect (99.9%) as the colour subsystem is absolutely crucial for the working of the robot and the demo. The objectives are the following: Close to perfect accuracy (99.9%) for color detection between red, orange, blue, yellow, and green Finding correct hue intervals for each colour, verifying the theoretical hue values of the five different colours (red, orange, blue, yellow, green) with real testing Finding saturation and brightness intervals for differentiating a bright colour with white/black Continuous sampling will be used during a time period of 10s for each colour, to ensure that colours are detected correctly, accurately and reliably.  |
| **Procedure** (numbered steps): | Sensor placed over a certain colour X Color detecting code placed in debugging mode (i.e. hue, brightness, saturation and colour detected are printed to the terminal) If no colour is detected or the detection switches periodically between colour detected and no colour detected, adjust the brightness and saturation lower bounds such that a colour is correctly detected.  If the colour detected is not the colour X or the detection switches periodically between colour X and another colour (not X), adjust the hue intervals such that the colour X is correctly detected Repeat step 2 three times, until colour X is correctly detected approximately 100% of the time.  Repeat the above three steps for all colours (blue, green, red, yellow, orange) |
...
| **Report/Outcomes**: | **Report/Outcomes:** Blue detected correctly ~100% of the time after one hue interval adjustment. Readings stabilized around hue 0.437–0.444 (≈157°–160°), saturation ~0.88, and brightness ~0.37. Green detected correctly ~100% of the time with no adjustments required. Readings clustered around hue 0.240–0.251 (≈86°–90°), saturation ~0.89–0.94, and brightness ~0.24–0.25. Yellow initially misclassified as orange due to overlapping hue intervals; the interval was narrowed to resolve the conflict. Following adjustment, yellow was detected correctly ~100% of the time, with readings stable around hue 0.128–0.131 (≈46°–47°), saturation ~0.878–0.902, and brightness ~0.318–0.326. Orange detected correctly ~100% of the time, with readings stable around hue 0.047–0.063 (≈17°–23°), saturation ~0.895–0.917, and brightness ~0.329–0.337. Red detected correctly ~100% of the time, with readings stable around hue 0.003–0.011 (≈1°–4°), saturation ~0.926–0.955, and brightness ~0.255–0.267. |
| **Conclusions** (for test- Pass/Fail? Specific quantities): | All five colours achieved detection accuracy at or exceeding the 99.9% threshold — overall **pass**. The colour subsystem is deemed accurate and reliable for use in the robot demo. The finalised HSV intervals determined through testing (using data in [Figure X](#figure-x)) are as follows: Colour Hue range (0–1) Hue range (°) Saturation lower bound Brightness lower bound Red 0.000 – 0.020 0° – 7° ≥ 0.92	 ≥ 0.25 Orange 0.040 – 0.070 14° – 25° ≥ 0.89 ≥ 0.32 Yellow 0.125 – 0.135 45° – 49° ≥ 0.87	 ≥ 0.31 Green 0.235 – 0.255 85° – 92 ≥ 0.88 ≥ 0.24 Blue 0.435 – 0.450 157° – 162° ≥ 0.88 ≥ 0.36  |
| **Subsequent actions to be taken**:Project Manager | The finalised HSV intervals and threshold values are to be integrated into the main robot codebase. Recalibration might be necessary if there are hardware changes and the height of the colour sensor changes.  |
| **Who is this for** (which leads/managers): | Arm Subteam LeadVision Subteam Lead |
<a id="table-13"></a>
...
| :---- | :---- |
| **People present/Testers**: | J. SauvéA. SavardZ. Chuang |
| **Author**: | A. Savard |
| **Hardware version**: | H 1.0 |
...
| **Procedure** (numbered steps): | Position robot at starting position. We determined this position to be fully up against the back wall, with the leftmost edge lining up with the left most edge of the orange door portion of the starting area. Angle here should be considered 0 degrees. Run main.py, with commands.txt setup for navigation and bed location of the first room. Record turned angle Ensure the robot detects the door of the room, record if not. Ensure the sweeper encompasses the whole room, and can find the bed at any location. Routinely move the bed to different edge and center cases through multiple tests. |
...
| :---- | :---- |
| **People present/Testers**: | J. SauvéS. GelfandZ. Chuang |
| **Author**: | A. Savard |
...
| :---- | :---- |
| **People present/Testers**: | M. BrookerI. Gaspart |
| **Author**: | M. Brooker |
| **Hardware version**: | H 1.0 |
| **Software version**: | S 0.4.0 |
...
| **Procedure** (numbered steps): | Position the robot at the threshold of the room entry door. Initiate the vision sweep routine upon room entry. Test Case A (Clear Path): Observe if the robot proceeds forward when no color is detected. Test Case B (Target Color): Place a green cube in the scan path; verify if the robot drops the cube and plays the assigned sound. Test Case C (Rejection/Exit): Place a non-green color in the scan path; verify if the robot executes a retreat maneuver through the door. Record the success of transitions between "scanning," "acting," and "retreating." Repeat 35 trials per case to ensure vision consistency. |
