...
MAX_ROOM_DISTANCE = 80  # centimeters of straight-line motion before robot can safely assume it is in a room
SWEEPS_PER_SWEEP = 2  # number of full ROMs swept per call of Megamind._sweep()
FW_PER_SWEEP = 9  # centimeters of straight-line motion between every sweep
MAX_ROOM_DEPTH = (
    35  # centimeters of max forward movement during sweeping before giving up on a room
)
TURN_DEADZONE = (
...
class Megamind(Processor):
    ...
    def _go_with_sensors(self, distance, speed=MIN_SPEED):
        ...
        if not (self.initial_orientation is None):
            ...
            initial_angle = gyro.queue.get(True)
            initial_angle = initial_angle.get("angle")
